import { getLogMetadata, getAllLogsMetadata } from '@/lib/logs';
...
export default async function Page({
  ...
  const metadata = getLogMetadata(slug);
  
  // Read and parse the MDX file
  const logsDirectory = path.join(process.cwd(), 'content/logs');
  const filePath = path.join(logsDirectory, `${metadata.filename}.mdx`);
    ...
    <article className="max-w-4xl mx-auto py-8">
      ...
      <div className="text-muted-foreground leading-relaxed">
        <ReactMarkdown 
          ...
          components={{
            h1: ({children}) => <h1 className="text-2xl font-semibold text-foreground mb-4 mt-8">{children}</h1>,
            ...
            blockquote: ({children}) => <blockquote className="border-l-4 border-muted pl-4 italic text-muted-foreground/80 mb-4">{children}</blockquote>,
            a: ({href, children}) => <a href={href} className="text-primary hover:text-primary/80 underline transition-colors">{children}</a>,
          }}
        >
          {content}
        </ReactMarkdown>
...
export async function generateMetadata({
  ...
  return {
    ...
    description: `${metadata.category} log from ${metadata.date}`,
